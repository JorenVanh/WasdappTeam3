...
public class InformatieRepositoryTest {
  ...
  private InformatieRepository informatieRepository;
    ...
    @Test
    public void createObjectTestFail() throws SQLException, NoQueryPossibleException{
         //init data
        when(resultSet.getString(InformatieRepository.KEY)).thenReturn("titel");
        when(resultSet.getString(InformatieRepository.LOCATIE)).thenReturn("locatie");
        when(resultSet.getString(InformatieRepository.STRAAT)).thenReturn("straat");
        ...
        Informatie result = informatieRepository.createObject(resultSet);
        //verify the result
        assertNotEquals("title", result.getTitel());
        assertNotEquals("locaties", result.getLocatie());
        assertNotEquals("straten", result.getStraat());
        assertNotEquals("nummers", result.getNummer());
        assertNotEquals("postcodes", result.getPostcode());
        assertNotEquals("gemeenten", result.getGemeente());
        assertNotEquals("landen", result.getLand());
        assertNotEquals("omschrijvingen", result.getOmschrijving());
        assertNotEquals("wikipedia_linken", result.getWiki_link());
        assertNotEquals("websites", result.getWebsite());
        assertNotEquals("telefoons", result.getTelefoon());
        assertNotEquals("emails", result.getEmail());
        assertThat(2.0, CoreMatchers.not(CoreMatchers.equalTo(result.getPrijs())));
